            ...
            <div class="dropdown-menu dropdown-list dropdown-menu-right">
              <div class="dropdown-header">Notifikasi
                <div class="float-right">
                  <a href="#" onclick="markAllAsReadDokter()">Tandai semua terbaca</a>
                </div>
              </div>
              <div class="dropdown-list-content dropdown-list-icons" id="notification-list-dokter">
                @if(isset($notifications) && $notifications->count() > 0)
                  @foreach($notifications as $notification)
                  <a href="{{ $notification->action_url ?? '#' }}" 
                     class="dropdown-item {{ !$notification->is_read ? 'dropdown-item-unread' : '' }}"
                     onclick="markAsReadDokter({{ $notification->id }})">
...
<script>
function markAsReadDokter(notificationId) {
    fetch(`/nakes/notifications/mark-read/${notificationId}`, {
        method: 'POST',
        headers: {
    ...
    .then(data => {
        if (data.success) {
            ...
            updateNotificationBadgeDokter();
        }
    })
    .catch(error => console.error('Error:', error));
}

function markAllAsReadDokter() {
    fetch('/nakes/notifications/mark-all-read', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
...
function updateNotificationBadgeDokter() {
    fetch('/nakes/notifications/unread-count')
    .then(response => response.json())
    .then(data => {
...
setInterval(updateNotificationBadgeDokter, 30000);
</script>